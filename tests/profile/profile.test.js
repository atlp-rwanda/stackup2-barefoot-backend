    ...
    with an error message and status code of 401`, (done) => {
      chai
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.be.an('object');
          expect(res.body).to.have.property('error').to.equal(customMessages.notAllowedToAccessThisResources);
          done();
        });
    });
    ...
    a user to verify his/her account first`, (done) => {
      chai
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.be.an('object');
          expect(res.body).to.have.property('error').to.equal(customMessages.verifyYourAccount);
          done();
        });
    });
